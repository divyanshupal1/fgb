  ...
  return (
    <div className='w-full bg-primary px-3 py-2 flex items-center justify-between  min-h-[60px]'>
        ...
        {pathname!="/login" && <div className='hidden sm:flex items-center justify-between space-x-2'>  
                <NavLink url='/' active={pathname=='/'?true:false} title="Market & News"/>
                <NavLink url='/' active={pathname=='/education '?true:false} title="Education"/>
...
const NavLink = ({title,active=false,url="/",className}) => {
  return(
    <Link className={`px-6 py-2 ${active?"bg-primary-light":""} font-bold rounded-full text-base ${className} hover:bg-primary-light`} href={url}>{title}</Link>
  )
}